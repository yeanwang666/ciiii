...
TARGET_TEST_DIR="${REPO_ROOT}/tests/ci/cases/tests"

usage() {
  cat <<'EOF' >&2
用法: templates/add_ci_case.sh <case_name> [display_name]

示例:
  templates/add_ci_case.sh file_io_extra
  templates/add_ci_case.sh file_io_extra "file-io-extra"
...
2. 在 tests/ci/suite.toml 中追加如下条目以注册用例:
