...
echo "[file-io-basic] 构建 host 版本" >&2
cargo build --manifest-path "${CRATE_DIR}/Cargo.toml" --release --bin "${BINARY_NAME}"

if [[ ! -x "${HOST_BIN}" ]]; then
  echo "[file-io-basic] 构建后未找到主机可执行文件 ${HOST_BIN}" >&2
  exit 1
fi
...
if [[ "${SKIP_DISK_IMAGE:-0}" == "1" ]]; then
  echo "[file-io-basic] SKIP_DISK_IMAGE=1，跳过写入磁盘镜像" >&2
  exit 0
fi

if [[ ! -f "${DISK_IMAGE}" ]]; then
  echo "[file-io-basic] 未找到磁盘镜像：${DISK_IMAGE}" >&2
  exit 1
fi

if [[ ! -w "${DISK_IMAGE}" ]]; then
  echo "[file-io-basic] 当前用户无权写入磁盘镜像 ${DISK_IMAGE}" >&2
  exit 1
fi

if ! command -v debugfs >/dev/null 2>&1; then
  echo "[file-io-basic] 未检测到 debugfs，请安装 e2fsprogs" >&2
  exit 1
fi

if ! rustup target list --installed | grep -q "^${TARGET_TRIPLE}$"; then
  echo "[file-io-basic] 安装 Rust 目标 ${TARGET_TRIPLE}" >&2
  rustup target add "${TARGET_TRIPLE}"
fi

echo "[file-io-basic] 构建 ${TARGET_TRIPLE} 版本" >&2
cargo build \
...
if [[ ! -f "${TARGET_BIN}" ]]; then
  echo "[file-io-basic] 未找到交叉编译产物 ${TARGET_BIN}" >&2
  exit 1
fi

echo "[file-io-basic] 写入磁盘镜像 -> ${DEST_PATH}" >&2
